...
export default function ProductActions({ product, waMsg }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {/* Main CTA buttons — WhatsApp stays largest (deliberate CRO decision, see CLAUDE.md);
          Telegram and Instagram are peers, so they share identical padding/text size/icon size/shape,
          differing only in each platform's own brand color. Always stacked (no lg:flex-row): this
          column lives inside max-w-6xl, so it's capped at ~536px at ANY viewport width — three buttons
          need ~670px+ to sit in a row without wrapping, so a row layout was never actually viable here,
          just less visibly broken on very narrow columns where flex-col already applied anyway. */}
      <div className="flex flex-col gap-3">
        <a href={`https://wa.me/966508949041?text=${waMsg}`} target="_blank" rel="noopener noreferrer"
          className="flex items-center justify-center gap-3 bg-green-700 hover:bg-green-600 text-white font-bold py-4 px-6 rounded-xl transition-all duration-300 hover:shadow-xl hover:shadow-green-500/30 text-lg">
        ...
        <a href="https://t.me/BillyStore1" target="_blank" rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-5 rounded-xl transition-all duration-300 text-base">
        ...
        <a href="https://ig.me/m/Billy_Store3" target="_blank" rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 bg-gradient-to-l from-purple-600 to-pink-600 hover:opacity-90 text-white font-bold py-3.5 px-5 rounded-xl transition-all duration-300 text-base">
      ...
      <div className="grid grid-cols-3 gap-2">
        <button onClick={() => copy('name')}
          className="flex items-center justify-center gap-2 min-h-11 bg-surface border-2 border-black/15 hover:border-brand/50 text-ink hover:text-brand font-bold py-2.5 px-3 rounded-xl transition-all text-xs">
          {copied === 'name' ? (
            <><span className="text-green-600">✓</span> تم النسخ</>
          ) : (
        ...
        <button onClick={() => copy('msg')}
          className="flex items-center justify-center gap-2 min-h-11 bg-surface border-2 border-black/15 hover:border-green-500/50 text-ink hover:text-brand font-bold py-2.5 px-3 rounded-xl transition-all text-xs">
          {copied === 'msg' ? (
            <><span className="text-green-600">✓</span> تم النسخ</>
          ) : (
